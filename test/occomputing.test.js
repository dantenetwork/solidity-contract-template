...
contract('OCComputing', function(accounts) {
    ...
    describe('Receive OC Computing Task', function() {
        it('should execute successfully', async () => {
            ...
            let argument = [1, 'PLATONEVMDEV', OCComputing.address, owner, [], to, action, calldata, [0, utils.toHexString(utils.stringToByteArray('0x11111111'))], 0];
    ...
    describe('Receive OC Computing Task Callback', function() {
        it('should execute successfully', async () => {
            ...
            let calldata = {items: [item]};
            let argument = [2, 'PLATONEVMDEV', OCComputing.address, owner, [], to, action, calldata, [1, '0x11111111'], 0];
            await crossChain.receiveMessage(argument, {from: user1});
