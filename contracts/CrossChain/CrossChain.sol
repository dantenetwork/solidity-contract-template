...
abstract contract CrossChain is ICrossChain, Ownable {  
  ...
  */
  ...
  function executeMessage(string calldata _chainName, uint256 _id) external {
    ...
    bytes memory data = abi.encodeWithSignature("verify(string,string,string)", _chainName, message.action, message.sender);
    (bool verifySuccess,) = message.contractAddress.call(data);
    if (!verifySuccess) {
      revert("verify failed");
    }

    currentSimplifiedMessage = SimplifiedMessage(message.id, message.fromChain, message.sender, message.signer, message.contractAddress, message.action);
